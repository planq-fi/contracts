...
import { PlanqFiPoolSwapTest } from '../typechain/PlanqFiPoolSwapTest'
...
describe('PlanqFiPool arbitrage tests', () => {
      ...
      for (const passiveLiquidity of [
        ...
        describe(`passive liquidity of ${formatTokenAmount(passiveLiquidity)}`, () => {
          const arbTestFixture = async ([wallet, arbitrageur]: Wallet[]) => {
            ...
            const testerFactory = await ethers.getContractFactory('PlanqFiPoolSwapTest')
            const tester = (await testerFactory.deploy()) as PlanqFiPoolSwapTest
          ...
          let tester: PlanqFiPoolSwapTest
